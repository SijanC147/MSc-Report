...
Some of the initial work making use of neural networks to learn word embeddings include \citep{collobert2008}, later refined in \citet{collobert2011}. Their approach consisted of a feedforward neural network tasked with predicting the correct word within a context window of five words including the target word itself.
...
\begin{figure}[!ht]
	...
	\caption{The differences between the model architectures proposed by Mikolov in \citep{mikolov2013}. The Continuous Bag-Of-Words (CBOW) approach predicts a word from its context and, conversely, the skip-gram model predicts the context from a word \citep{mikolov2013b}.}
...
Using co-occurrence statistics to extract continuous representations of words within a large corpus of data has been explored in NLP in works as early as \citet{rumelhart1988}, as cited by Bojanowski \citep{bojanowski2017}
