...
As far back as 1996, before the popularity of pre-trained word embeddings such as \textit{GloVe} and \textit{Word2Vec}, Gallwitz et al. \cite{gallwitz1996} pointed out that using a single token is somewhat crude. It could not possibly encompass the wealth of linguistic information expressed by every OOV word that is encountered; consider that an OOV word can be anything from a spelling mistake to proper noun, such as the name of an entity, and anything in between.
