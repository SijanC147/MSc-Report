...
\item \texttt{--run-config} Values which are passed on to the Tensorflow run configuration. These include options such as setting a random seed for an experiment and how often to save model summaries during training.\footnote{The full specification is accessible at \\https://www.tensorflow.org/versions/r1.12/api\_docs/python/tf/estimator/RunConfig}
...
\begin{code}
	python -m tsaplay.task batch --batch-file="batch_tasks.txt"
\end{code}
